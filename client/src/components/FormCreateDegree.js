...
const notification = {
    ...
    admin_role: 'Please create new user to using this function',
}
...
class Form extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            notificationStatus: 0, // 1: successed, -1: failed
            listIdBlock: [],
        }
    ...
    async handleCancel() {
        ...
        if (this.state.listIdBlock.length !== 0 && blockID.length !== 0) {
    ...
    async handleSubmit(event) {
        ...
        if (userdata.public_key === " ") {
            this.setState(refreshFormSubmit({ headerTitle: notification.admin_role, notificationStatus: -1, isNotification: false }, originalStateObject));
            return;
        }
        ...
        if (transaction) {
            ...
            this.setState(refreshFormSubmit({ headerTitle: notification.success, notificationStatus: 1, listIdBlock, isNotification: true }, originalStateObject));
        } else {
            this.setState(refreshFormSubmit({ headerTitle: notification.fail, isNotification: false }, originalStateObject));
        }
    }
    ...
    render() {
        const notiStatus = this.state.notificationStatus;
        return (
            <form className="form--create-degree" onSubmit={this.handleSubmit}>
                <h5 className={notiStatus === 1 ? 'is-success'  : (notiStatus === 0 ? '' : 'is-fail')}>{this.state.headerTitle}</h5>
                <div>
